...
def get_base_embeddings(trainmode, unkdtokens, tg_start, sentence):
    pw_i = dy.parameter(w_i)
    pb_i = dy.parameter(b_i)

    pw_bi = dy.parameter(w_bi)
    pb_bi = dy.parameter(b_bi)
    ...
    dist_x = [dy.scalarInput(abs(i - tg_start) + 1) for i in xrange(sentlen)]

    baseinp_x = [(pw_i * dy.concatenate([emb_x[j], pos_x[j], dist_x[j]]) + pb_i) for j in xrange(sentlen)]

    if USE_WV:
        pw_e = dy.parameter(w_e)
        pb_e = dy.parameter(b_e)
    ...
    basebi_x = [dy.rectify(pw_bi * dy.concatenate([basefwd[eidx], baserev[sentlen - eidx - 1]]) + pb_bi) for eidx in
                xrange(sentlen)]

    if USE_DEPS:
        pw_di = dy.parameter(w_di)
        pb_di = dy.parameter(b_di)
        ...
        baseinp_x = [dy.rectify(pw_di * dy.concatenate([dhead_x[j], dheadp_x[j], drel_x[j], basebi_x[j]]) + pb_di) for j in
                     xrange(sentlen)]
        basebi_x = baseinp_x
...
def get_deppath_embeddings(sentence, embpos_x):
    ...
    for spath in set(sentence.shortest_paths.values()):
        ...
        pw_p = dy.parameter(w_p)
        pb_p = dy.parameter(b_p)
        pathlstm = dy.rectify(pw_p * dy.concatenate([pathfwd[-1], pathrev[-1]]) + pb_p)
...
def get_cpath_embeddings(sentence):
    ...
    for phrpath in set(sentence.cpaths.values()):
        ...
        pw_cp = dy.parameter(w_cp)
        pb_cp = dy.parameter(b_cp)
        cpathlstm = dy.rectify(pw_cp * dy.concatenate([cpathfwd[-1], cpathrev[-1]]) + pb_cp)
...
def get_factor_expressions(fws, bws, tfemb, tfdict, valid_fes, sentence, spaths_x=None, cpaths_x=None):
    pw_z = dy.parameter(w_z)
    pb_z = dy.parameter(b_z)
    pw_f = dy.parameter(w_f)
    pb_f = dy.parameter(b_f)
    ...
    for j in xrange(sentlen):
        ...
        for i in xrange(istart, j + 1):
            ...
            for y in valid_fes:
                ...
                factexprs[fctr] = pw_f * dy.rectify(pw_z * fbemb_ijy + pb_z) + pb_f
                # if USE_DROPOUT:
...
def get_constit_loss(fws, bws, goldspans):
    ...
    pw_fb = dy.parameter(w_fb)
    pb_fb = dy.parameter(b_fb)
    pw_c = dy.parameter(w_c)
    pb_c = dy.parameter(b_c)
    ...
    for j in xrange(sentlen):
        ...
        if USE_SPAN_CLIP and j > ALLOWED_SPANLEN: istart = max(0, j - ALLOWED_SPANLEN)
        for i in xrange(istart, j + 1):
            constit_ij = pw_c * dy.rectify(pw_fb * dy.concatenate([fws[i][j], bws[i][j]]) + pb_fb) + pb_c
            logloss = dy.log_softmax(constit_ij)

            isconstit = int((i, j) in goldspans)
            losses.append(dy.pick(logloss, isconstit))
